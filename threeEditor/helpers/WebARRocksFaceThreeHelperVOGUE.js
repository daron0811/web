...
const WebARRocksFaceThreeHelper = (function(){
  ...
  function init_three(maxFacesDetected){
    ...
    _three.camera = new THREE.PerspectiveCamera(_cameraFoVY, that.get_viewAspectRatio(), 0.1, 5000);
    ...
    for (let i=0; i<maxFacesDetected; ++i){
      ...
      faceFollowerParent.visible = true;
  ...
  function draw_video(){
    ...
    //DaRon 畫面選項
    _gl.uniform1f(_shps.copyCrop.uniforms.BSIGMA, 0.05); //Daron : 美肌強度參數 預設0.01 數字越小越接近原本預設圖片，越大則會變模糊
    _gl.uniform1f(_shps.copyCrop.uniforms.brightness, 0.15);//Daron : 亮度參數 預設0 範圍0~1
    _gl.uniform1f(_shps.copyCrop.uniforms.contrast, 1.1);//Daron : 對比參數 預設1 範圍 -3~3
    _gl.uniform1f(_shps.copyCrop.uniforms.saturation, 1.1);//Daron : 飽和參數 預設1 範圍 -3~3
    _gl.uniform1f(_shps.copyCrop.uniforms.mapSizeX, 800); //Daron :  圖片的長寬比，實際輸出的畫面會切成4:3，但填太小會有橫線，故填800*600大小
    _gl.uniform1f(_shps.copyCrop.uniforms.mapSizeY, 600);
    _gl.uniform1i(_shps.copyCrop.uniforms.LUTmap, 0);
  ...
  function process_faceSlot(detectState, slotIndex){
    ...
    if (detectState.isDetected) {
      ...
      faceSlot.visible = true;      
    } else if (faceSlot.visible){
      faceSlot.visible = false;
  ...
  function init_shps(){
    ...
    varying vec2 vUV2;\n\
    uniform sampler2D uun_source;\n\
    uniform float brightness;// 亮度= 0 \n\
    uniform float contrast; // 對比= 1  \n\
    uniform float saturation;// 飽和= 1 \n\
    uniform float BSIGMA;// 0.1\n\
    uniform float mapSizeX;\n\
    uniform float mapSizeY;\n\
    #define SIGMA 10.0\n\
    #define MSIZE 15\n\
    const bool USE_CONSTANT_KERNEL = true;\n\
    ...
    float normalizeColorChannel(in float value, in float min, in float max) {return (value - min)/(max-min);}\n\
    vec4 SkinSmooth()\n\
    {\n\
        vec3 c = texture2D(uun_source, vUV).rgb;\n\
        ...
        vec3 cc;\n\
        float factor;\n\
        float bZ = 0.39894/BSIGMA;\n\
        for(int i=-ksize; i <= ksize; ++i){\n\
        for (int j=-ksize; j <= ksize; ++j){\n\
        vec2 offset;\n\
        offset.x = float(i)/mapSizeX;\n\
        offset.y = float(j)/mapSizeY;\n\
        cc = texture2D(uun_source,vUV + offset).rgb;\n\
        factor = normpdf3(cc-c, BSIGMA)*bZ*kernel[ksize+j]*kernel[ksize+i];\n\
        Z += factor;\n\
        final_colour += factor*cc;}}    \n\
        vec4 fragColor = vec4(final_colour/Z,1.0);\n\
        return fragColor;\n\
    }\n\
    uniform sampler2D LUTmap;\n\
    vec3 LookUpTable(vec3 imageColor,sampler2D LUTMap) //LUT,會有破面而且圖片做翻轉,暫時先保留 \n\
    {\n\
        float target =  texture2D(LUTMap,vUV).a;\n\
        if(target == 0.0)\n\
        {\n\
            return imageColor;\n\
        }\n\
        ...
        vec3 lookUpColor_1 = texture2D(LUTMap,tableUV_1).rgb;\n\
        vec3 lookUpColor_2 = texture2D(LUTMap,tableUV_2).rgb;\n\
    ...
    void main() {\n\
        //vec3 color = LookUpTable(SkinSmooth().rgb,LUTmap);\n\
    ...
    //取得Shader的Uniform
    _shps.copyCrop.uniforms.BSIGMA = _gl.getUniformLocation(_shps.copyCrop.program, 'BSIGMA'); //美肌
    _shps.copyCrop.uniforms.brightness = _gl.getUniformLocation(_shps.copyCrop.program, 'brightness'); //亮度
    _shps.copyCrop.uniforms.contrast = _gl.getUniformLocation(_shps.copyCrop.program, 'contrast'); //對比
    _shps.copyCrop.uniforms.saturation = _gl.getUniformLocation(_shps.copyCrop.program, 'saturation'); //飽和
    _shps.copyCrop.uniforms.mapSizeX = _gl.getUniformLocation(_shps.copyCrop.program, 'mapSizeX'); //圖片尺寸
    _shps.copyCrop.uniforms.mapSizeY = _gl.getUniformLocation(_shps.copyCrop.program, 'mapSizeY'); //圖片尺寸
    
    _shps.copyCrop.uniforms.LUTmap = _gl.getUniformLocation(_shps.copyCrop.program, 'LUTmap'); //LUT圖片
    initImage(_gl);
  }
  ...
  function initImage(gl) {
    ...
    // 找不到圖片素材，用自己的大頭貼當作範例
    image.src = 'https://raw.githubusercontent.com/positlabs/spark-lut-patch/master/textures/neutral.png';
    
    image.onload = function() {
      setupTexture(gl, LutTexture, image);
    }
  }
  
  function setupTexture(gl, texture,image) {

    gl.bindTexture(gl.TEXTURE_2D, texture);
    gl.pixelStorei(gl.UNPACK_FLIP_Y_WEBGL, false); // 紋理座標垂直翻轉
    
    gl.texImage2D(gl.TEXTURE_2D, 0, gl.RGBA, gl.RGBA, gl.UNSIGNED_BYTE, image);//材質讀入記憶體
    ...
    if( !gl.isTexture(texture) ){console.log("Error: Texture is invalid");}

...
function MenubarComponent( editor ) {
	...
	title.setTextContent( 'Component');
	container.add( title );
	...
	var items = [
		{ title: 'Particle', file: 'particle1.txt' },
		{ title: 'Raycaster', file: 'raycast.txt' },
		{ title: '[AR]FaceInfo', file: 'faceInfo.txt' },
		{ title: 'Sprite Animation', file: 'spriteAnim.txt' }
	];
		...
		( function ( i ) {
			...
			option.onClick( function () {
				...
				if(isSelectItem())
				{
					...
					loader.load( 'components/' + item.file, function ( text ) {

						var script = { name: item.title, source: text };
	...
	// var option = new UIRow();
	// option.setClass( 'option' );
	// option.setTextContent( 'Occluders_litNose' );
	// option.onClick( function () {
	...
	function isSelectItem()
	{
		...
		{
			window.alert('Please Select A Target Object Item. \n請先選擇場景一個對象物件！');
			return false;
		}
		return true;
	}
